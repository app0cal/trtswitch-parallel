...
double est_psi_rpsftm(double psi,
  const Rcpp::IntegerVector& id,
  const Rcpp::IntegerVector& stratum,
  const Rcpp::NumericVector& time,
  const Rcpp::IntegerVector& event,
  const Rcpp::IntegerVector& treat,
  const Rcpp::NumericVector& rx,
  const Rcpp::NumericVector& censor_time,
  double treat_modifier,
  bool recensor,
  bool autoswitch,
  double target);

struct bootstrap_worker : public RcppParallel::Worker {
  //inputs (should only be reading these NOT editing them)
  const IntegerVector idn, stratumn, eventn, treatn;
  const NumericVector timen, rxn, censor_timen;
  const NumericMatrix zn;
  const std::function<
      Rcpp::List(IntegerVector&, IntegerVector&,
        NumericVector&, IntegerVector&,
        IntegerVector&, NumericVector&,
        NumericVector&, NumericMatrix&) > f;
  const int n, n0, n1;

  // outputs (write by index to avoid overwriting)
  RMatrix<double> hrhats;
  RMatrix<double> psihats;

  bootstrap_worker(
    const IntegerVector& idn, 
    const IntegerVector& stratumn,  
    const NumericVector& timen, 
    const IntegerVector& eventn,
    const IntegerVector& treatn,
    const NumericVector& rxn,
    const NumericVector& censor_timen,
    const NumericMatrix& zn,
    //const Function& f, (this didn't work as intended)
    const std::function<
      Rcpp::List(IntegerVector&, IntegerVector&,
        NumericVector&, IntegerVector&,
        IntegerVector&, NumericVector&,
    ...
    const int n0,
    const int n1,
    NumericMatrix& hrhats_out, 
    NumericMatrix& psihats_out
  ): idn(idn), stratumn(stratumn), timen(timen), eventn(eventn), treatn(treatn), rxn(rxn), censor_timen(censor_timen),zn(zn),
  f(f), n(n), n0(n0), n1(n1), hrhats(hrhats_out),psihats(psihats_out){}

  void operator()(std::size_t start, std::size_t end){
    int i, j;

    for(std::size_t k = start; k < end; ++k){
      //n here is individual worker
      IntegerVector idb(n), stratumb(n), eventb(n), treatb(n);
      NumericVector timeb(n), rxb(n), censor_timeb(n);
      NumericMatrix zb(n, zn.ncol());
      ...
      //NumericVector isn't safe for concurrency w RcppParallel so swapped to NumericMatrix
      hrhats(k,0) = as<double>(out["hrhat"]);
      psihats(k,0) = as<double>(out["psihat"]);
    }
  }
};
/* Commented out bc it was counting as a duplicate we just leave a signature
that we copy from rpsftm.cpp
...
// [[Rcpp::export]]
List rpsftm_parallelcpp(const DataFrame data,
  ...
  if (!boot) { // use log-rank p-value to construct CI for HR if no boot
    ...
    //bottom is modified bootstrapping with mutlithreadding from RcppParallel
    NumericVector hrvec(n_boot), psivec(n_boot);
    NumericMatrix hrhats_mat(n_boot,1);
    NumericMatrix psihats_mat(n_boot,1);

    bootstrap_worker worker(
      idn, stratumn, timen, eventn, treatn, rxn, censor_timen, zn, f, n, n0, n1, hrhats_mat, psihats_mat
    );
    parallelFor(0,n_boot,worker);
    
    for (int i = 0; i < n_boot; ++i) {
      hrvec[i] = hrhats_mat(i, 0);
      psivec[i] = psihats_mat(i, 0);
    }
