test_that("tsesimp_mt is repeatable for same seed, BUT different threads", {
  ...
  args1 <- args; args1$threads <- 1L
  args8 <- args; args8$threads <- 8L
  
  out1 <- do.call(trtswitch::tsesimp_mt, args1)
  out2 <- do.call(trtswitch::tsesimp_mt, args8)
